...
window.renderTranscriptList=function(){
  ...
  view.innerHTML=transcriptHeader('Saved conversations · open one to read the summary, transcript, and chat from that exact source.')+'<div class="val-transcript-stats"><span class="val-transcript-stat"><strong>'+Number(c.total||transcriptState.items.length)+'</strong> transcripts</span></div><div class="val-transcript-list">'+(rows||'<div class="val-empty">No transcripts are available yet. When meeting, voice, or VAL conversation transcripts are captured, they will appear here with their summary, source text, and transcript-bound chat.</div>')+'</div>';
};
