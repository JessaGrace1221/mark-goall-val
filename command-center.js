...
window.renderTranscriptList=function(){
  ...
  var c=transcriptState.counts,rows=transcriptState.items.map(function(t){var meta=[t.source,t.createdAt?new Date(t.createdAt).toLocaleString():''].filter(Boolean).join(' · '),summary=t.summary&&typeof t.summary==='object'?t.summary.executiveSummary:(t.summaryPreview||t.summary||t.preview||'Summary pending.');return '<article class="val-transcript-row"><div class="val-transcript-copy"><div class="val-transcript-row-head"><h3>'+safe(transcriptShortText(t.title,'Transcript',120))+'</h3></div><div class="val-transcript-meta">'+safe(meta)+'</div><p>'+safe(transcriptShortText(summary,'Open this transcript to review the meeting summary.',340))+'</p></div><div class="val-transcript-actions"><button class="val-ui-btn primary" onclick="openTranscriptDetail(\''+safe(t.id)+'\')">Open Transcript</button><button class="val-ui-btn" onclick="transcriptAskFromList(\''+safe(t.id)+'\')">Chat</button></div></article>';}).join('');
  view.innerHTML=transcriptHeader('Saved conversations · open one to read the summary, transcript, and chat about the meeting.')+'<div class="val-transcript-stats"><span class="val-transcript-stat"><strong>'+Number(c.total||transcriptState.items.length)+'</strong> transcripts</span></div><div class="val-transcript-list">'+(rows||'<div class="val-empty">No true meeting transcripts are available yet. VAL is filtering out task chats, meeting prep prompts, and planning conversations so this page only shows real source transcripts.</div>')+'</div>';
};
...
function renderTranscriptDetail(t){
  ...
  var debug='<details class="val-detail-card val-transcript-debug"><summary>Processing details</summary><h3>Status</h3><p>'+safe(status)+'</p><h3>Key Points</h3>'+detailList([s.clientSummary,s.internalNotes].concat(s.relationshipUpdates||[]).filter(Boolean),'No key points extracted.')+'<h3>Decisions</h3>'+detailList(s.keyDecisions,'No decisions extracted.')+'<h3>Action Items</h3>'+detailList(tasks,'No action items extracted.')+'<h3>Created Tasks</h3>'+detailList(createdTasks,'No tasks have been pushed to the main task system yet.')+'<h3>Participants & Match Confidence</h3>'+detailList(participants,'No participants detected.')+'<h3>Contact Updates</h3>'+detailList(updates,'No contact updates extracted.')+'<h3>Action Log</h3>'+detailList(log,'No actions logged.')+'<h3>Recap Draft</h3>'+recapHtml+'</details>';
  view.innerHTML=transcriptHeader(meta,true)+'<div class="val-transcript-detail"><div class="val-detail-main"><section class="val-detail-card"><div class="val-detail-actions"><button class="val-ui-btn primary" onclick="transcriptAskFocus()">Chat About This Meeting</button></div><h3>Summary</h3><p>'+safe(transcriptShortText(s.executiveSummary||s.clientSummary||'Summary pending.','Summary pending.',900))+'</p></section><section class="val-detail-card"><h3>Transcript</h3><p class="val-full-transcript">'+safe(t.transcriptText||t.rawTranscript||'No transcript text is available.')+'</p></section>'+debug+'</div><aside class="val-detail-side"><section class="val-detail-card"><h3>Chat About This Meeting</h3><div class="val-chat-log" id="valTranscriptChat"><div class="val-chat-msg">Ask about what happened, what was decided, what matters, or what VAL noticed in this transcript.</div></div><div class="val-chat-input"><input id="valTranscriptQuestion" placeholder="Ask VAL about this meeting" onkeydown="if(event.key===\'Enter\')transcriptAsk()"><button class="val-ui-btn primary" onclick="transcriptAsk()">Ask</button></div></section></aside></div>';
}
