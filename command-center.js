...
function transcriptHeader(subtitle,back){return '<div class="val-view-head"><div><h2>Transcript Intelligence</h2><p>'+safe(subtitle)+'</p></div><div class="val-view-actions">'+(back?'<button class="val-ui-btn" onclick="renderTranscriptList()">Inbox</button>':'')+'<button class="val-ui-btn" onclick="renderTranscriptReviewQueue()">Review Queue</button><button class="val-ui-btn" onclick="renderTranscriptIntakeStatus()">Intake Status</button><button class="val-ui-btn" onclick="recoverStoredTranscripts()">Recover Existing</button><button class="val-ui-btn" onclick="repairTranscriptProcessing()">Process Pending</button><button class="val-ui-btn" onclick="openIntegrationStatus()">Webhook Setup</button><button class="val-ui-btn" '+(transcriptState.loading?'disabled':'')+' onclick="loadTranscripts(true).catch(function(){})">'+(transcriptState.loading?'Refreshing…':'Refresh')+'</button></div></div>';}
...
window.recoverStoredTranscripts=function(){
  ...
  return fetcher((window.PROXY||'')+'/api/val/transcripts/recover-existing',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({days:3650,limit:100})}).then(function(data){
    var samples=(data.importedSamples||[]).map(function(x){return '<article class="val-review-card"><span class="val-status '+(x.processingError?'review':'ok')+'">'+(x.processingError?'Needs processing':'Recovered')+'</span><h3>'+safe(transcriptShortText(x.title||x.id,'Recovered transcript',110))+'</h3><p>'+safe((x.sourceType||'stored VAL record')+' · '+(x.characters||0)+' chars'+(x.processingError?' · '+x.processingError:''))+'</p></article>';}).join('');
    if(view)view.innerHTML=transcriptHeader('Recovered '+Number(data.imported||0)+' stored transcript'+(Number(data.imported||0)===1?'':'s')+'.',true)+'<div class="val-transcript-stats"><span class="val-transcript-stat"><strong>'+Number(data.candidates||0)+'</strong> candidates found</span><span class="val-transcript-stat"><strong>'+Number(data.imported||0)+'</strong> imported</span><span class="val-transcript-stat"><strong>'+Number(data.skipped||0)+'</strong> skipped</span><span class="val-transcript-stat"><strong>'+Number((data.errors||[]).length)+'</strong> errors</span></div><div class="val-review-grid">'+(samples||'<div class="val-empty">No recoverable transcript-shaped records were found in VAL storage.</div>')+'</div>';
...
window.renderTranscriptList=function(){
  ...
  view.innerHTML=transcriptHeader('Only real transcript records appear here: meeting, call, voice, upload, or webhook captures with raw transcript text.')+'<div class="val-transcript-stats"><span class="val-transcript-stat"><strong>'+Number(c.total||transcriptState.items.length)+'</strong> transcripts</span><span class="val-transcript-stat"><strong>'+Number(c.failedProcessing||0)+'</strong> processing issues</span></div><div class="val-transcript-list">'+(rows||'<div class="val-empty"><strong>No real transcripts are available yet.</strong><br>VAL has not received a usable meeting, call, voice, upload, or webhook transcript for this dashboard. Planning notes, prompts, drafts, and task artifacts are intentionally hidden here.</div>')+'</div>';
};
